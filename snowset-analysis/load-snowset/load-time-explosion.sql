...
begin;
create table HourlyTimeExplosion
(
    queryid  bigint,
    time     timestamp,
    fraction float
...
with SamplesPerQuery as (select queryid, count(*) as totalSamples
                         ...
                         group by queryid)
insert
into HourlyTimeExplosion (select t.queryid, date_trunc('hour', t.time), count(*) * 1.0 / max(s.totalSamples)
    from TimeExplosion t,
    SamplesPerQuery s
    where t.queryid = s.queryid
    group by t.queryid, date_trunc('hour', t.time));
commit;
...
-- ---------------------------------------------------------------------------------------------------------------------
-- Load week table_te (contains a random sample of the week_te table for faster expxloration)
begin;
create table week_te as (select *
    from queries_te TABLESAMPLE SYSTEM(1));
commit;

-- ---------------------------------------------------------------------------------------------------------------------
-- queries_te indexes
create index on queries_te (queryid);
create index on queries_te (warehouseid);
create index on queries_te (databaseid);

-- ---------------------------------------------------------------------------------------------------------------------
-- day_te indexes
create index on day_te (queryid);
create index on day_te (warehouseid);
create index on day_te (databaseid);

-- ---------------------------------------------------------------------------------------------------------------------
-- week_te indexes
create index on week_te (queryid);
create index on week_te (warehouseid);
create index on week_te (databaseid);
